...
if menu == "인원수 변화":
    ...
    if df is not None:
        ...
        # 11월 1일 이전 평균 예측
        before_nov = df_full[df_full["날짜"] < "2025-11-01"]["인원수(명)"].mean()
        df_full["인원수(명)"] = df_full["인원수(명)"].fillna(before_nov)

        df_full["변화량"] = df_full["인원수(명)"].diff()
        avg_change = df_full["변화량"].mean()
        ...
        fig.update_layout(font=dict(family="Malgun Gothic, sans-serif"))
...
# ===============================
    ...
    with st.spinner("활동 데이터를 집계 중입니다..."):
...
# ===============================
    ...
    with st.spinner("관리진 목록을 불러오는 중입니다..."):
        df = load_admin_data()
...
# ===============================
    ...
    with st.spinner("이벤트 데이터를 불러오는 중입니다..."):
        df = load_event_data()
...
# ===============================
    ...
    with st.spinner("내전 승률을 분석 중입니다..."):
        df = load_match_data()

    if df is not None:
        win_rate = df["승리팀"].value_counts(normalize=True) * 100

        st.subheader("⚔️ 팀별 승률")
